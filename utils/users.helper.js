...
export const handleInsertUser = async user => {
    try {
        ...
        await database.query(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            `,
            [user.user_id, user.email, user.firstName, user.lastName, user.password, user.phoneNumber, user.avatar],
        );
        await database.end();
    } catch (error) {
...
export const handleGetUsers = async (page, sortBy, sortOrder, limit, offset) => {
