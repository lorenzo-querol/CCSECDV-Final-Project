...
export const handleInsertUser = async (user) => {
	try {
		...
		throw new Error(
			`[${new Date().toLocaleString()}] handleInsertUser: ${error.message}`,
		);
	}
};
...
export const handleDeleteUser = async (user_id) => {
	try {
		...
		await database.query("DELETE FROM reports WHERE user_id = ?", [user_id]);

		await database.end();
	} catch (error) {
		throw new Error(
			`[${new Date().toLocaleString()}] handleDeleteUser: ${error.message}`,
		);
	}
};
...
export const handleUpdateUser = async (user_id, updatedInfo) => {
	try {
		...
		throw new Error(
			`[${new Date().toLocaleString()}] handleUpdateUser: ${error.message}`,
		);
	}
};
...
export const handleGetLikedPosts = async (user_id) => {
	try {
		...
		throw new Error(
			`[${new Date().toLocaleString()}] handleGetMyPosts: ${error.message}`,
		);
	}
};
