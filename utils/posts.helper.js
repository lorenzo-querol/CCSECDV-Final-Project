	...
	try {
		...
		await database.query(
			`
                ...
                VALUES (?, ?, ?, ?, ?)
            `,
			[post.post_id, post.user_id, post.name, post.description, post.image],
		);
		await database.end();
	} catch (error) {
		throw new Error(
			`[${new Date().toLocaleString()}] handleInsertPost: ${error.message}`,
		);
	}
};
...
export const handleUpdatePost = async (post_id, heart_count) => {
	try {
		...
		throw new Error(
			`[${new Date().toLocaleString()}] handleUpdatePost: ${error.message}`,
		);
	}
};
...
export const handlePostDelete = async (post_id) => {
	try {
		...
		throw new Error(
			`[${new Date().toLocaleString()}] handlePostDelete: ${error.message}`,
		);
	}
};
