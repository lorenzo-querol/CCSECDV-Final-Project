...
export default function Register() {
   ...
   const [userData, setUserData] = useState({}); // State to store user data
    ...
    // Function to fetch user data and set it to the state
    const fetchUserData = async () => {
        try {
            const response = await axios.get("/api/users/:userId"); // Replace ":userId" with the actual user ID or identifier
            if (response.data.ok) {
                setUserData(response.data.user); // Assuming the response contains the user object
            }
        } catch (error) {
            // Handle error
    ...
    // Call the fetchUserData function when the component mounts to fetch the user data
    useEffect(() => {
        fetchUserData();
    }, []);
