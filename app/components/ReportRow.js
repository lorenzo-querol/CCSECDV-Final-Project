...
const getStatus = status => {
    switch (status) {
        case 'pending':
            return <div className="px-2 py-1 bg-yellow-500 rounded-lg">Pending</div>;
        case 'approved':
            return <div className="px-2 py-1 bg-green-500 rounded-lg">Approved</div>;
        case 'rejected':
            return <div className="px-2 py-1 bg-red-500 rounded-lg">Rejected</div>;
        case 'completed':
            return <div className="px-2 py-1 bg-indigo-500 rounded-lg">Completed</div>;
    }
};
    ...
    return (
        <tr>
            <td
                ...
                onClick={() => {
                    handleNameClick(report.post_id);
                }}
            >
                {report.name}
            </td>
            ...
            <td className="px-4 py-2 border-b">
                <div className="flex items-center justify-center ">{getStatus(report.status)}</div>
            </td>
                ...
                {!report.cooldown_until && (
                    <div className="flex items-center justify-center space-x-2">
                        ...
                        <input
                            ...
                            value={timeValue}
                            onChange={event => handleTimeValueChange(event.target.value)}
                        />
                        <select
                            name="timeUnit"
                            className="p-2 bg-transparent border-b-2 border-indigo-500 outline-none w-28"
                            value={timeUnit}
                            onChange={event => handleTimeUnitChange(event.target.value)}
                        >
            ...
            <td className="px-4 py-2 border-b">
                {report.cooldown_until && new Date(report.cooldown_until).toLocaleString()}
            </td>
            <td className="px-4 py-2 border-b">
                {!report.cooldown_until && (
                    <div className="flex items-center justify-center space-x-2">
                        <button
                            ...
                            onClick={event =>
                                handleSubmit(event, 'approved', `${timeValue}_${timeUnit}`, report.report_id)
                            }
                        >
                        ...
                        <button
                            ...
                            onClick={event =>
                                handleSubmit(event, 'rejected', `${timeValue}_${timeUnit}`, report.report_id)
                            }
                        >
